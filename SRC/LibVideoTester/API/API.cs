...
public static class VideoTesterApi
{
    ...
    /// <param name="metaDataProvider">by Default, it will use the FFProbeMetaDataProvider, but you can create your own for custom behaviour.</param>
    /// <param name="deserializer"><by default, will use the FFProbeMetaDataToVideoInfo class, but you can create your own for custom behaviour (for example if you mutate the incoming data somehow from standard ffprobe output)./param>
    /// <returns>A VideoInfo object populated with the information it was able to retrieve. Don't assume that if you get an object, that the readings are valid though.</returns>
    ...
    /// <summary>
    /// Get all configuration files for in a specified directory, by default it will use the "Configurations" directory.
    /// </summary>
    /// <param name="directoryToCheck">The path to check, by default it will only look for .json files.</param>
    /// <param name="fileProvider">By default will use JsonFileProvider, but you can create your own if you want to serve files from non disk locations.</param>
    /// <param name="deserializer">By default will use NewtonSoftJsonDeserializer, but you can create your own for custom behaviours.</param>
    /// <returns>A dictionary of configurations, where the key is the path to the file containing the config, and the value is the Configuration object generated from it.</returns>
    ...
    /// <summary>
    /// A easy to use helper method to that works as a oneline to get all matching configurations for a given video
    /// </summary>
