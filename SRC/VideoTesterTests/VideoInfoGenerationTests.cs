...
namespace VideoTesterTests {
  ...
  public class VideoInfoGenerationTests {
    ...
    public void shouldReturnValidVideoInfo() {
      VideoMetaDataFactory generator =
          new VideoMetaDataFactory(new DummyMetaDataGenerator(), new FFprobeMetaToVideoInfo());
      VideoMetaData v = generator.GetVideoInfoAsync("c:/foo")
                            .GetAwaiter()
                            .GetResult();  // the file path doesn't matter
      Assert.IsNotNull(v);
      Configuration c = new Configuration("Sample Config", new string[] { "h264" }, 3840, 2160,
                                          new int[] { 25 }, 23205);
