...
namespace VideoTesterTests {
  public class APITests {
    ...
    [SetUp]
    public void Setup() { _fileProvider = new MockFileProvider(DummyData); }
    ...
    public void shouldReturnValidVideoMetaData() {
      VideoMetaData data =
          VideoTesterApi.GetVideoMetaDataAsync("/path", new DummyMetaDataGenerator())
              .GetAwaiter()
              .GetResult();
    ...
    public void shouldGetValidConfigurations() {
      Dictionary<string, Configuration> configuration =
          VideoTesterApi.GetConfigurationsAsync("Configurations", _fileProvider)
              .GetAwaiter()
              .GetResult();
    ...
    public void shouldMatchAConfigurationUsingSimplePath() {
      Dictionary<string, Configuration> results =
          VideoTesterApi
              .ExtractMetaDataAndFindConfigMatchesAsync("/path", new DummyMetaDataGenerator(),
                                                        _fileProvider)
              .GetAwaiter()
              .GetResult();
      Assert.AreEqual(1, results.Keys.Count);
