...
namespace VideoTester {
  class Program {
    ...
    static async Task<int> Main(string[] args) {
      ...
      // if there aren't enough args just force a help command. Couldn't find a way to make this the
      // default behaviour.
      ...
      var fileOption = new Option<string>(
          name: "-i", description: "the input stream to check against, for example -i movie.mov");

      var rootCommand = new RootCommand(
          "Video Sanity Testing Tool. It will check a video against some known configurations you have specified in the Configurations folder. ");

      rootCommand.AddOption(fileOption);
      rootCommand.SetHandler(async (path) => { await CheckVideo(path); }, fileOption);
    ...
    static async Task CheckVideo(string pathToVideo) {
      ...
      log.Information(
          "Meta data retrieved from file width:{width}, height:{height}, bitrate:{bitrate}, fps: {fps}, codec: {codec}",
          v.Width, v.Height, v.BitrateKPBS, v.FrameRate, v.Codec);
